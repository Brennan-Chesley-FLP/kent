...
- **Transport selection** from ``scraper.driver_requirements``: no browser
  ...
  ``HttpxTransport``); a camoufox requirement (``CFCAP_HANDLER``,
  ``HCAP_HANDLER``, or ``RCAP_HANDLER``) → :class:`CamoufoxTransport`; any
...
- **Browser-profile auto-resolution** from ``$JKENT_HOME/profiles/{name}``
  with the CLI's precedence (a camoufox requirement → camoufox, else
  ``FF_ALIKE`` → firefox, else ``CHROME_ALIKE`` → chrome). Unlike the CLI, a
  missing profile directory is a warning, not an error — the unified engines
  run fine profile-less.
...
def resolve_browser_profile(
    ...
    *,
    ...
    CLI-equivalent precedence: a camoufox requirement (``CFCAP_HANDLER``,
    ``HCAP_HANDLER``, or ``RCAP_HANDLER``) always gets the camoufox profile
    (camoufox is the only engine that reliably passes CF managed challenges,
    hCaptcha, and reCAPTCHA), else ``FF_ALIKE`` → firefox, else
    ``CHROME_ALIKE`` → chrome. A scraper with none of these (or a

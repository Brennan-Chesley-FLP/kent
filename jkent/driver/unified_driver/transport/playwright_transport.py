...
from jkent.driver.unified_driver.requirements import CAMOUFOX_REQUIREMENTS
...
class _InlineArchive:
    """An archive the browser rendered inline instead of downloading.

    Firefox (camoufox) opens PDFs in its built-in pdf.js viewer and never fires
    a ``download`` event — even with ``pdfjs.disabled`` /
    ``browser.download.open_pdf_attachments_inline`` set (a FF bug we can't pref
    our way around). When the archive click navigates the page to the file
    rather than downloading it, we reconstruct the archive from the bytes of
    that navigation's own response, carried here.
    """
...
class PlaywrightTransport(Transport[WorkerPage], Recoverable):
    ...
    async def _resolve_archive(
        ...
        # downloading it: stage the captured response bytes to a temp file so
        # the rest of the pipeline (FileArchiveStream + finish_archiving's
        # unlink) is identical to a real download.
        if isinstance(result, _InlineArchive):
            tmp_path = await asyncio.to_thread(
                self._stage_inline_temp_file, result.body
            )
    ...
    async def _execute_via_download(
        ...
        case an :class:`_InlineArchive` carrying the navigation's response bytes
        is returned instead. See :meth:`_await_download_or_inline`.
    ...
    async def _await_download_or_inline(
        ...
        * navigation first       -> the browser rendered the file inline, so we
          read the bytes off the captured response and return an
          :class:`_InlineArchive`.
        ...
        download_task: asyncio.Task[Download] = asyncio.ensure_future(
            page.wait_for_event("download", **wait_kwargs)
        )
        nav_task: asyncio.Task[Any] = asyncio.ensure_future(
            page.wait_for_event(
                "framenavigated",
        ...
        try:
            # Let both waiters attach their one-shot listeners before the
            # trigger can produce the event they're waiting for.
            await asyncio.sleep(0)
            try:
                await trigger()
            except PlaywrightTimeoutError:
                # An inline navigation can trip the click's own auto-wait for
                # "scheduled navigations"; the waiters below are the real
                # arbiter of what actually happened, so don't fail here.
                pass

            done, _ = await asyncio.wait(
                {download_task, nav_task},
                return_when=asyncio.FIRST_COMPLETED,
            )

            if download_task in done and download_task.exception() is None:
                return download_task.result()

            if nav_task in done and nav_task.exception() is None:
                ...
                inline = await self._capture_inline_archive(captured)
                if inline is not None:
                    return inline

            exc = download_task.exception() if download_task.done() else None
            raise TransientException(
                "Archive trigger produced neither a download nor a readable "
                f"inline render{f': {exc}' if exc else ''}"
            )
        finally:
            with contextlib.suppress(Exception):
                page.remove_listener("response", _on_response)
            for task in (download_task, nav_task):
                if not task.done():
                    task.cancel()
            await asyncio.gather(
                download_task, nav_task, return_exceptions=True
            )

    @staticmethod
    async def _capture_inline_archive(
        captured: list[PlaywrightResponse],
    ) -> _InlineArchive | None:
        """Reconstruct an inline-rendered archive from a captured response.

        Prefers the most recent captured file response; skips any whose body is
        unavailable (e.g. Playwright can't surface it). Returns ``None`` when no
        captured response yields bytes.
        """
        for resp in reversed(captured):
            try:
                ...
                continue
            if body:
                return _InlineArchive(
                    body=body, url=resp.url, status_code=resp.status
                )
        return None

    @staticmethod
    def _stage_inline_temp_file(body: bytes) -> str:
        """Write inline-render bytes to a temp file; return its path.

        Mirrors the temp file a Playwright download stages to, so the caller can
        hand back a :class:`FileArchiveStream` that ``finish_archiving`` unlinks
        exactly like a real download.
        """
    ...
    def _build_engine(self) -> BrowserEngine:
        """Pick the engine: camoufox for CFCAP/HCAP/RCAP scrapers, playwright otherwise.

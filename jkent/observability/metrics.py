...
effectively nothing until the host (en_banc) installs an SDK. See
``EN_BANC_OTEL.md`` for the host side.
...
High-cardinality identifiers (``flow_run_id``) deliberately do NOT go on the
...
class _Instruments:
    ...
    def __init__(self) -> None:
        ...
        # Per-run state (attrs: scraper, flow_run_id). Gauges, so the last
...
_labels: contextvars.ContextVar[dict[str, str] | None] = (
    contextvars.ContextVar("jkent_metric_labels", default=None)
)

...
``flow_run_id`` is read from OpenTelemetry **baggage**, which the host sets
...
def flow_run_id() -> str | None:
    """The host-provided flow-run id from baggage, or None if unset."""
    value = baggage.get_baggage("flow_run_id")
...
def request_span(*, scraper: str, step: str | None = None) -> Iterator[Span]:
    ...
    Stamps ``scraper`` / ``flow_run_id`` (and ``step`` when known) as span
    ...
    with tracer().start_as_current_span("jkent.request") as span:
        ...
        fid = flow_run_id()
        if fid is not None:
            span.set_attribute("jkent.flow_run_id", fid)
        try:
            yield span
        finally:

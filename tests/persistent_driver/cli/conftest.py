...
@pytest.fixture
async def populated_db(initialized_db, db_path: Path) -> Path:
    ...
    async with session_factory() as session:
        for request_id, result_type, data, is_valid, errors in result_data:
            await session.execute(
                ...
                ) VALUES (:request_id, :result_type, :data_json, :is_valid, :validation_errors_json, datetime('now'))
                """),
                {
                    "request_id": request_id,
    ...
    async with session_factory() as session:
        for (
            ...
            await session.execute(
                ...
                ) VALUES (:request_id, :error_type, :message, :selector, :selector_type, :expected_min, :expected_max, :actual_count, :is_resolved, :resolution_notes, datetime('now'), :request_url, :error_class, :traceback)
                """),
                {
                    "request_id": request_id,
